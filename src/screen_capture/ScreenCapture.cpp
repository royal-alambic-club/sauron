...
namespace OutputCapture
{
    ...
    ScreenCapture *OutputCapture::ScreenCapture::pInstance = nullptr;

    bool ScreenCapture::CaptureFullScreen(std::string outputFileName) const
    {
        const std::tuple<uint16_t, uint16_t> screenSize = this->GetDesktopResolution();

        return this->StartCapture(0, 0, std::get<0>(screenSize), std::get<1>(screenSize), outputFileName.c_str());
    }

    int ScreenCapture::GetFilePointer(HANDLE FileHandle) const
    {
        return SetFilePointer(FileHandle, 0, 0, FILE_CURRENT);
    }

    bool ScreenCapture::SaveBMPFile(const char *filename, HDC bitmapDC, uint16_t width, uint16_t height) const
    {
        HBITMAP OffscrBmp = NULL; // bitmap that is converted to a DIB0
    ...
    bool ScreenCapture::StartCapture(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const char *filename) const
    {
    ...
    const std::tuple<uint16_t, uint16_t> ScreenCapture::GetDesktopResolution() const
    {
        uint16_t width = GetSystemMetrics(SM_CXSCREEN);
        uint16_t height = GetSystemMetrics(SM_CYSCREEN);
